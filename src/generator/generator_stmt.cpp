...
std::shared_ptr<VisitorResult> Generator::VisitASTAssignStmt(ASTAssignStmt *node) {
    ...
    llvm::Value *r_value = right->getValue();;
    if (PascalType::isEqual(left->getType(), right->getType())) {
        if (right->getMem()->isIntegerTy() && left->getMem()->isFloatingPointTy()) {
            r_value = this->builder.CreateSIToFP(r_value, llvm::Type::getFloatTy(this->context));
        }
    }
    this->builder.CreateStore(r_value, left->getMem());
    return nullptr;
}
...
std::shared_ptr<VisitorResult> Generator::VisitASTRepeatStmt(ASTRepeatStmt *node) {
    ...
    for (auto expr: node->getStmtList()) {
        expr->Accept(this);
    }
