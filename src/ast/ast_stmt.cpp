...
ASTStmt::ASTStmt(ASTNonLabelStmt *non_label_stmt, int label = -1)
...
void ASTStmt::Print(GraphGenerator *g) {
  if (label == -1) {
    ...
    g->addNode("label_stmt " + label_str, this->line(), this->col());
    non_label_stmt->Print(g);
    g->Pop();
  }
}

ASTAssignStmt::ASTAssignStmt(std::ASTExpr *expr1, ASTExpr *expr2)
...
ASTProcStmt::ASTProcStmt(std::string id, ASTExprList *expr_list)
...
ASTIfStmt::ASTIfStmt(ASTExpression *expression, ASTStmt *stmt,
                     ASTElseClause *else_clause)
    : expression(expression), stmt(stmt), else_clause(else_clause) {}
...
void ASTCaseStmt::Print(GraphGenerator *g) {
  ...
  this->expr_->Print(g);
  this->case_expr_list->Print(g);
  g->Pop();
}

ASTCaseExprList::ASTCaseExprList() {}

ASTCaseExprList::add_case_expr(ASTCaseExpr *expr) {
...
ASTGotoStmt::ASTGotoStmt(std::string label) { label_ = atoi(label.c_str()); }
